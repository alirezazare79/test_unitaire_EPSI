...
from src.language.EN import LangueEn
from src.language.FR import LangueFr
            ...
            for cas_lang in cas_langue:
                ...
                for time in attendu.keys():
                    with self.subTest((cas, cas_lang, time)):
                        cas_resultat = PalindromeBuilder().set_langue(cas_lang[0]).set_moment(time).build().palindrome(
                            cas).split(os.linesep)
    ...
    def test_palindrome_au_revoir_langue(self):
        for cas in cas_palindrome_valide_invalide:
            for cas_lang in cas_langue:
                ...
                with self.subTest(cas):
                    cas_resultat = PalindromeBuilder().set_langue(cas_lang[0]).build().palindrome(cas).split(
                        os.linesep)
